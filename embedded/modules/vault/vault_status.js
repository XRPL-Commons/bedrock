...
/**
 * XRPL Smart Vault Status Module
 *
 * This module handles querying a vault ledger entry on XRPL networks.
 *
 * Usage: node vault_status.js <config-json-path>
 *
 * Config JSON format:
 * {
 *   "vault_id": "AAAA...",
...
/**
 * Query a vault's status on XRPL network
 */
async function vaultStatus(config) {
  const log = config.verbose ? console.error.bind(console) : () => {};

  log('Querying vault status on XRPL...\n');
  ...
  try {
    ...
    if (isLocal) {
      ...
      const ledgerEntry = await httpRPC(rpcUrl, 'ledger_entry', { vault: config.vault_id }, 10000);

      log('Ledger entry response received');
      vaultData = ledgerEntry.node || ledgerEntry;
    } else {
      ...
      try {
        ...
        log('Ledger entry response received');
        vaultData = ledgerEntry.result.node || ledgerEntry.result;

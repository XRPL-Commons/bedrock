...
/**
 * XRPL Smart Vault Deposit Module
 *
 * This module handles depositing into a smart vault on XRPL networks via VaultDeposit transactions.
 *
 * Usage: node vault_deposit.js <config-json-path>
 *
 * Config JSON format:
 * {
 *   "vault_id": "AAAA...",
...
/**
 * Deposit into a smart vault on XRPL network
 */
async function vaultDeposit(config) {
  const log = config.verbose ? console.error.bind(console) : () => {};

  log('Depositing into vault on XRPL...\n');
  ...
  try {
    ...
    // Build VaultDeposit transaction

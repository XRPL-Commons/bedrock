...
/**
 * XRPL Smart Escrow Cancel Module
 *
 * This module handles cancelling a smart escrow on XRPL networks.
 *
 * Usage: node escrow_cancel.js <config-json-path>
 *
 * Config JSON format:
 * {
 *   "owner": "rXXX...",
...
/**
 * Cancel a smart escrow on XRPL network
 */
async function cancelEscrow(config) {
  const {
    owner,
    escrow_sequence,
    network_url,
  ...
  log('Cancelling smart escrow on XRPL...\n');
  ...
  try {
    ...
    log('\nSubmitting EscrowCancel transaction...');

...
func runModify(cmd *cobra.Command, args []string) error {
	...
	// Updating the contract code requires the matching ABI (function table);
	// without it the ledger rejects ContractModify with temARRAY_EMPTY.
	// Default to the conventional abi.json (as generated by deploy) when --abi
	// is omitted alongside --wasm.
	if modifyWasm != "" && modifyABI == "" {
		modifyABI = "abi.json"
	}
	if modifyWasm != "" {
		if _, err := os.Stat(modifyABI); err != nil {
			return fmt.Errorf("updating --wasm requires its ABI: %q not found (pass --abi <file>)", modifyABI)
		}
	}
